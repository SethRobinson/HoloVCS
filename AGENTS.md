...
- 3DS MULTIVIEW GIF MAKER (Aug 2026): `tools\make_holo_gif.ps1` builds looping parallax-sweep,
  smooth-cutaway, and buildup (full-cut -> build -> sweep -> pause -> quick teardown) GIFs
  for ANY running 3DS game in multiview mode, from the core's native-res quilt dumps (no
  window capture; native 400x240 output by default). Usage, -MaxCut probing method, and the
  MSR depth-layout numbers: docs/automation_workflow.md "3DS multiview GIF maker".
...
- 3DS (IN PROGRESS, Aug 2026): `azahar_libretro.dll`, built from Seth's Azahar fork at
  ...
  ROUND 6 (Aug 2026): on LKG builds the 3DS now defaults to TRUE MULTIVIEW (capture
  mode 2) - the core renders one view per lens tile and delivers a packed quilt the
  sprite path blits per tile; verified on the Go at 60 fps. `-holobands` forces the
  band diorama for A/B. Read the doc before touching any of it.
