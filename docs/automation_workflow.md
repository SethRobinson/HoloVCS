...
- Layers: L1 = full play-area BG (opaque ground plane), L2+L3 = obstacle extrusion (masked nametable re-render), L3 also gets play-area sprites, L4 = HUD plus HUD-strip sprites (the minimap blip and item icons are sprites; without that strip they would hide behind the opaque HUD).
