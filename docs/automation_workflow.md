...
- Screen-scroll transitions stay fully 3D via the vendored FCEUmm core's optional `retro_set_holo_bg_tile_filter` extension. The old save-state nametable edit failed because Zelda writes incoming rows while the visual replay is already running. The new 256-tile allow mask is applied when the PPU fetches each live nametable tile, after those writes and before the CHR lookup, without changing nametable or serialized state. The authoritative ground/state pass is always unfiltered; each obstacle replay enables its mask for exactly one `retro_run` and then clears it. A stale core DLL without the export keeps the legacy stationary filter and falls back to 2D only during scrolls. Verified with horizontal and vertical transition video plus mode-6 mid-scroll `dumplayers` output, Aug 2026: raised layers stayed filtered with no full-width stripe and no depth pop on arrival.
