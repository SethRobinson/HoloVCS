...
#define _PC        g_NESX.PC
#define _A         g_NESX.A
#define _X         g_NESX.X
#define _Y         g_NESX.Y
#define _S         g_NESX.S
#define _P         g_NESX.P
#define _PI        g_NESX.mooPI
#define _DB        g_NESX.DB
#define _count     g_NESX.count
#define _tcount    g_NESX.tcount
#define _IRQlow    g_NESX.IRQlow
#define _jammed    g_NESX.jammed
...
void X6502_Init(void) {
	...
	memset((void*)&g_NESX, 0, sizeof(g_NESX));
	for (x = 0; x < 256; x++)
		if (!x)
			ZNTable[x] = Z_FLAG;
		else if (x & 0x80)
