...
void DrawNSF(uint8 *localXBuf) {
	char snbuf[16];
	int x;

	if (vismode == 0) return;

	memset(localXBuf, 0, 256 * 240);
	...
	DrawTextTrans(localXBuf + 10 * 256 + 4 + (((31 - strlen((char*)NSFHeader.SongName)) << 2)), 256, NSFHeader.SongName, 6);
	DrawTextTrans(localXBuf + 26 * 256 + 4 + (((31 - strlen((char*)NSFHeader.Artist)) << 2)), 256, NSFHeader.Artist, 6);
	DrawTextTrans(localXBuf + 42 * 256 + 4 + (((31 - strlen((char*)NSFHeader.Copyright)) << 2)), 256, NSFHeader.Copyright, 6);

	DrawTextTrans(localXBuf + 70 * 256 + 4 + (((31 - strlen("Song:")) << 2)), 256, (uint8*)"Song:", 6);
	sprintf(snbuf, "<%d/%d>", CurrentSong, NSFHeader.TotalSongs);
	DrawTextTrans(localXBuf + 82 * 256 + 4 + (((31 - strlen(snbuf)) << 2)), 256, (uint8*)snbuf, 6);
