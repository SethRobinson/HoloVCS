...
static bool update_option_visibility(void)
{
   ...
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      ...
      if (opt_showAdvSystemOptions != opt_showAdvSystemOptions_prev)
      {
         ...
         localsize = sizeof(options_list) / sizeof(options_list[0]);
         for (locali = 0; locali < localsize; locali++)
         {
            option_display.key = options_list[locali];
...
static void check_variables(bool startup)
{
   ...
   //Seth's addition to let the host control to render BG/sprites
   var.key = "fceumm_video_flags";

   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
       FCEUI_SetRenderPlanes(var.value[1] == '1', var.value[0] == '1');
      
       gNoBGFillColor = 0;

       if (var.value[0] != '1')
       {
           //also check if we should set the bg render color, this will only take effect if no bg is rendered
           char* pTemp = &var.value[2];
           gNoBGFillColor = 0; //default
           while (*pTemp != 0)
           {
               if (*pTemp == '|')
               {
                   //found it?
                   gNoBGFillColor = atoi(++pTemp);
                   //done, but will keep processing
               }
...
void FCEUD_Update(uint8 *localXBuf, int32 *Buffer, int Count)
{
}
