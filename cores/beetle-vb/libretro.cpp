...
extern "C"
{
    extern void Reset3DLayers();
    extern void ClearLayerRGBASurface(int layerID);
    extern void ClearLayerRGBASurfaces();

    retro_log_printf_t log_cb;
    Layer3DInfo* g_pLayer3DInfo = NULL;
    bool g_SethRenderToScreen = true;
    int g_holo_3d_layer_count = 5;
}



static retro_video_refresh_t video_cb;
static retro_video_refresh_ex_t video_ex_cb;
...
static void check_variables(void)
{
   ...
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      ...
      else if (strcmp(var.value, "3dlayered") == 0)
          setting_vb_3dmode = VB3DMODE_3DLAYERED;
   ...
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
       ...
       if (len > 1)
       {
           if (var.value[0] == '1')
           {
               g_SethRenderToScreen = true;
           }
           else
           {
               g_SethRenderToScreen = false;
           }
       }
   }

   var.key = "holo_3d_layer_count";
   ...
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
       ...
       if (len > 0)
       {
           ...
           g_holo_3d_layer_count = atoi(var.value);
       }
   }
...
bool retro_load_game(const struct retro_game_info *info)
{
   ...
   g_pLayer3DInfo = (Layer3DInfo*) malloc(sizeof(Layer3DInfo));

   memset(g_pLayer3DInfo, 0, sizeof(Layer3DInfo));
   if (!g_pLayer3DInfo)
   {
       return false;
   }

   g_pLayer3DInfo->m_layerCount = g_holo_3d_layer_count;
   g_pLayer3DInfo->m_pLayers = (Layer3DSlice*)malloc(sizeof(Layer3DSlice)* g_pLayer3DInfo->m_layerCount);
   memset(g_pLayer3DInfo->m_pLayers, 0, sizeof(Layer3DSlice) * g_pLayer3DInfo->m_layerCount);
...
void retro_run(void)
{
   ...
   int pitch = FB_WIDTH << 2;
   
   //SETH Video callback is here
   //for showing a custom layer rather than the default one, we can enable this below

   /*
   for (int i = 0; i < C_MAX_3D_LAYERS; i++)
   {
       if (g_pLayer3DInfo->m_pLayers[i].m_bUsed)
       {
           pix = (uint32*) &g_pLayer3DInfo->m_pLayers[i].m_image[0];
           pitch = g_pLayer3DInfo->m_pLayers[i].m_pitchBytes;


           //test code to fill it with bytes
          //
          // int counter = 0;
          // for (int x = 0; x < g_pLayer3DInfo->m_pLayers[i].m_width; x++)
          //     for (int y = 0; y < g_pLayer3DInfo->m_pLayers[i].m_height; y++)
          //     {
          //         counter++;
          //         if (counter % 4 == 0)
          //         {
          //             pix[x + (y * (pitch/4))] = MAKECOLOR(255, 0, 0, 255);
          //         }
          //         else
          //         {
          //             pix[x + (y * (pitch/4))] = MAKECOLOR(255, 0, 255, 255);
   ...
   */ 
   ...
   //seth's badass version.  Or just bad maybe
   if (g_SethRenderToScreen)
   {
       video_ex_cb(pix, width, height, pitch, g_pLayer3DInfo);
   }
   else
   {
       //we've been asked to not render video, probably to skip frames
   }

   ClearLayerRGBASurfaces();
...
void retro_set_video_refresh_ex(retro_video_refresh_ex_t cb)
{
    video_ex_cb = cb;
}
